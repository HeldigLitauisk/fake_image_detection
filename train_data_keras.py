...
TRANSFER_LEARNING = [ResNet50, VGG19, InceptionV3, MobileNetV2][1]
...
def create_transfer_learning_model():
    ...
    model.add(TRANSFER_LEARNING(
        include_top=False, pooling=POOLING, classes=NUMBER_OF_CLASSES))
...
def create_train_data(train_dir, data_name='train'):
    npy_data_data = './data/{}_data.npy'.format(data_name)
    ...
    for img in tqdm(os.listdir(train_dir)):
        ...
        training_data.append([image, np.array(label)])
...
def main():
    ...
    scores = model.evaluate(train_y, test_y, verbose=1)
    ...
    fig = plt.figure()
    for num, data in enumerate(validation_data[:24]):
        img_num = data[1]
